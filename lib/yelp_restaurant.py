...
# business_category = spark.read.option("header", "true").csv('/home/hadoop/yelp_dataset_challenge_academic_dataset/business_category.csv')
...
# output = spark.sql("""select * from business limit 10""").toPandas()

def write_json_file(output, filename):
	output.to_json(filename, orient='records')
